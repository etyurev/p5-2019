...
class Quality:
    ...
    def plot_stuff(self, array):
        smooth_z = self.z_smoothvalue
        #smooth_z = self.moving_Avg(array[:][:,2], 3)

        plt.plot(array[:][:,0], smooth_z)


        #plt.plot(self.create_tangent_line(array[:][:,0][-1], array[:][:,0][0], len(array))[0], self.create_tangent_line(array[:][:,0][-1], array[:][:,0][0], len(array))[1])
        #try:
        #    plt.plot(self.final_tangent_array[:][:,0],self.final_tangent_array[:][:,2])
        #except Exception as e:
        #    pass
        #plt.plot(self.ax, self.az, 'ro')
        #plt.plot(self.bx, self.bz, 'ro')
        #plt.plot(self.cx, self.cz, 'ro')
    ...
    def derivative_min_max(self):
        derivmin = np.min(self.dydx)
        derivmax = np.max(self.dydx)
    ...
    def get_width_and_midpoint(self):
        ...
        #print("x1: ", x1, " x2: ", x2, " midpoint: ", ((x2+1)/2))
