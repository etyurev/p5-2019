...
class PointCloud:
    ...
    def getPointCloudLines(self):
        ...
        for point in np.asarray(self.pcd.points):
            ...
            if y != holder:
                ...
                #print(self.pointCloudLine[-1][0][1])
                npArr.clear()
            npArr.append(point)
    ...
    def getParameters(self):
        ...
        self.i_max, i, self.y_range, line, k = 0, 0, 0, [], 0
        for points in self.pcd.points:
            if holder != points[1]:
    ...
    def pcd_to_image(self, span=0.5):
        ...
        """
        self.getParameters()
        matrix = np.zeros(self.i_max*len(self.pointCloudLine))
        matrix = np.reshape(matrix, (len(self.pointCloudLine), -1))
        self.getPointCloudLines()
    ...
    def mask_image(self, img, npArr, span=0.5):
        ...
        matrix = np.reshape(matrix, (len(self.pointCloudLine), -1))
        yArr = []
        for i in range(len(self.pointCloudLine)):
